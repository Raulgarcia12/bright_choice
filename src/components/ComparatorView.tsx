  ...
  if (products.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-border p-12 text-center">
        ...
        <p className="text-lg font-medium text-muted-foreground">{t('noProductsToCompare', language)}</p>
        <p className="mt-1 text-sm text-muted-foreground/70">{t('addProducts', language)}</p>
      </div>
    );
  }
              ...
              <CardHeader className="pb-3">
                ...
                {badges.length > 0 && (
                  <div className="flex flex-wrap gap-1 pt-1">
                    {badges.map((b) => (
                      <Badge key={b.label} className="gap-1 bg-green-100 text-green-800 hover:bg-green-200 text-xs">
                        {b.icon}{b.label}
                      </Badge>
                    ))}
                  </div>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="rounded-lg bg-muted/50 p-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xl font-bold" style={{ color: scoreColor }}>{score.total}</span>
                  </div>
